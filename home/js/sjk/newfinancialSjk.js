(function($){
	var fin = {
		init: function(){
			...
			this.url = '/Home/Index/newfinancial_json?name=' + this.name ;
			this.category = '';
			this.dataArr = {};
		...
		renderDom: function (data) {
			...
			self.getData(self.url, function(data){
				...
				self.data.forEach(function(ele){
					ele['a103'] ? self.bg.push({'time': ele['dtime'], 'href': ele['a103']}) : '';
					ele['a104'] ? self.zgs.push({'time': ele['dtime'], 'href': ele['a104']}) : '';
				})
		...
		showLiEcharts: function(tar, data){
			...
			var title2 = data['name'] + dataType[ydata][0]
			if(!ydata2){
				self.curIndex = tar.index()
				self.title = self.setting.title || (title2 + '数据图')
				showEChart(data, dataType[ydata][0], '', data[ydata], data[ydata2], coin, danwei, self.myChart, self.screenShot, self.title, self.setting.bgImg || self.syImg);
			}else{

				self.curIndex = tar.index()
				self.title = self.setting.title || (title2 + '及同比增长率数据图')
				showEChart(data, dataType[ydata][0], '同比增长率', data[ydata], ydata2, coin, danwei, self.myChart, self.screenShot, self.title, self.setting.bgImg || self.syImg);
			}
		...
		renderCpyNav: function(type, data, curCpy) {
			...
			arr.forEach(function(item) {
				$('.cpy').append('<li><a class="' + (item['name'] == curCpy ? 'active' : '') + '" href="https://www.100ec.cn/Home/User/newFinancialData.html?name='+ item['name'] + '">' + item['name'] + '</a></li>')
			})
		},
		bindEvent: function () {
			var self = this;
