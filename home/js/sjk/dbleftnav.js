...
function isScrollBar(h) {
	var boxH = $('.dsj-nav-box').height()
	var navH = h || $('#dsjNav').children(':first').outerHeight()
	if(navH > boxH) {
		// console.log($('.scrollBarBox'))
...
function changeScrollBar(zongH, barH) {
	var h = barH / zongH * barH;
	var t = (-($('#dsjNav').children(':first').offset().top - $(window).scrollTop() -55) / (zongH - barH)) * (barH - h);
	$('.bar').css({
		'height': h + 'px',
		'top': t + 'px',
	})
...
function showUl(data, tar, baseUrl) {
	...
	if(flag) {
		...
		oLi.append(oP)
		render(data.children, oLi, data.baseUrl || baseUrl )
	}
...
function render(data, tar, baseUrl) {
